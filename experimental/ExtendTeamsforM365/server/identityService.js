...
export async function initializeIdentityService(app) {
  ...
  app.post("/api/validateAadLogin", async (req, res) => {
    try {
      ...
      console.log(
        `Error in /api/validateAadLogin handling: ${error.statusMessage}`
      );
      res
        .status(error.status)
        .json({ status: error.status, statusText: error.statusMessage });
    }
  });
...
async function validateApiRequest(req, res, next) {
  ...
  console.log(
    "Request for bot, validation will be performed by Bot Framework Adapter"
  );
  next();
  // } else {
...
async function validateAadLogin(req, res) {
  ...
  if (aadUserId) {
    ...
    if (employeeId) {
      ...
      throw {
        status: 404,
        statusMessage: "Employee ID not found for this user",
      };
    }
  } else {
  ...
  if (employeeIdCache[aadUserId]) {
    ...
    try {
      ...
      if (graphResponse.ok) {
        ...
        console.log(
          `Error ${graphResponse.status} calling Graph in getEmployeeIdForUser: ${graphResponse.statusText}`
        );
      }
    } catch (error) {
...
export async function getOboAccessToken(clientSideToken, scopes) {
  ...
  try {
    ...
    if (oboResponse.status !== 200) {
      ...
      if (
        oboData.error.toLowerCase() === "invalid_grant" ||
