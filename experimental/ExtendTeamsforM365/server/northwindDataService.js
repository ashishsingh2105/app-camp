...
export async function getEmployee(employeeId) {
  ...
  result.city = `${employee.City}, ${employee.Region || ""} ${
    employee.Country
  }`;

  const employeeOrders = orders.data.filter(
    (order) => order.EmployeeID === result.id
  );
...
export async function getOrders() {
  const orders = [];
  orders.push(getOrder(10247));
  orders.push(getOrder(10248));
  orders.push(getOrder(10249));
  orders.push(getOrder(10250));
  orders.push(getOrder(10251));
  orders.push(getOrder(10252));
  orders.push(getOrder(10253));
  orders.push(getOrder(10254));
  orders.push(getOrder(10255));
  orders.push(getOrder(10256));
...
export async function getOrder(orderId) {
  ...
  console.log(`WOW: ${JSON.stringify(order)}`);
  const customer = customers.item(order.CustomerID);
...
export async function getCategory(categoryId) {
  ...
  const productsInCategory = products.data.filter(
    (product) => product.CategoryID == categoryId
  );

  result.products = productsInCategory
    .map((product) => {
      const supplier = suppliers.item([product.SupplierID]);
      return {
...
export async function getProduct(productId) {
  ...
  const details = orderDetails.data.filter(
...
export async function getProductByName(productNameStartsWith) {
  ...
  const matchingProducts = products.data.filter((item) =>
    item.ProductName.toLowerCase().startsWith(match)
  );
...
export async function updateOrderQuantity(orderId, productId, newQuatity) {
  console.log(`orderId: ${orderId} quantity: ${newQuatity}`);
  var orderDetails = await db.getTable("Order_Details", "OrderID");
  var orders = orderDetails.data.filter((item) => item.OrderID == orderId);
  var order = orders.filter((item) => item.ProductID == productId);
  console.log(`Before: ${JSON.stringify(order[0])}`);
  order[0].Quantity = newQuatity;
  console.log(`${order.Quantity}: New order: ${JSON.stringify(order)}`);
  orderCache[orderId] = null;
  await orderDetails.save();

  // testing purpose
  const orderDetails1 = await db.getTable("Order_Details", "OrderID");
  const orders1 = orderDetails.data.filter((item) => item.OrderID == orderId);
  orders1.forEach((item) =>
    console.log(`verbose logging ${item.ProductID} : ${item.Quantity}`)
  );
}
